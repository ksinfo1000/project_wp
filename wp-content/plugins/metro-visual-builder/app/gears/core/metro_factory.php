...
class Metro_Factory
{
	...
	function load_assets()
	{
		...
		else
		{
			...
			if( $color_scheme != 'none' )
			{
				wp_register_style(
					'metro_builder_skin',
					$this->app_url.'/assets/css/'.$color_scheme.'/style.css'
				);
	...
	public function parse_mvb_array( $arr )
	{
		...
		foreach( $arr as $row )
		{
			...
			$str .= '<section class="row-wrapper '.$row['settings']['cssclass'].'"'.$row_css.'>  <div class="'.$the_row_class.' '.$row['settings']['cssclass'].'" style="'.$my_row_css.'">';
			$str .= $totop_button;
			foreach( $row['columns'] as $column )
			{
