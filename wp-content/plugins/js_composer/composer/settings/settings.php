    ...
    public function init() {
        ...
        $tab_prefix = '_element_css';
         ...
         add_settings_section($this->option_group.$tab_prefix,
            ...
            $this->page.$tab_prefix);
        add_settings_field(self::$field_prefix.'row_css_class', __("Row CSS class name", "js_composer"), array(&$this, 'row_css_class_callback'), $this->page.$tab_prefix, $this->option_group.$tab_prefix);
        add_settings_field(self::$field_prefix.'column_css_classes', __("Columns CSS class names", "js_composer"), array(&$this, 'column_css_classes_callback'), $this->page.$tab_prefix, $this->option_group.$tab_prefix);
         /**
         * Custom CSS
         */
...
<div class="wrap vc-settings" id="wpb-js-composer-settings">
    ...
    <?php if($tab=='element_css'): ?>
        ...
        <div class="deprecated">
            <p>
            <?php _e("<strong>Deprecated:</strong> To override class names that are applied to Visual Composer content elements you should use WordPress add_filter('vc_shortcodes_css_class') function. <a class='vc_show_example'>See Example</a>.") ?>
            </p>
        </div>
        <div class="vc_helper">
        <?php
...
<?php
    ...
    public static function buildCustomColorCss() {
         ...
         *
         */
        $filename = WPBakeryVisualComposer::getInstance()->assetPath('css/js_composer_front_custom.css');
        $use_custom = get_option(self::$field_prefix.'use_custom');
        if(!$use_custom) {
    ...
    public static function buildCustomCss() {
         ...
         * Building css file.
         */
        $filename = WPBakeryVisualComposer::getInstance()->assetPath('css/custom.css');
        $css_string = '';
        $custom_css_string = get_option(self::$field_prefix.'custom_css');
        if(!empty($custom_css_string)) $css_string .= $custom_css_string;
        if ( ! $wp_filesystem->put_contents( $filename, $css_string, FS_CHMOD_FILE) ) {
            if ( is_wp_error($wp_filesystem->errors) && $wp_filesystem->errors->get_error_code() )
